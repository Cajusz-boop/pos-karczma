...
type PermissionsData = {
  ...
  operations?: {
    ...
    cancelItem?: boolean;
    cancelOrder?: boolean;
    editSentItem?: boolean;
    reprintKitchen?: boolean;
  };
  prohibitions?: {
    noQuantityChange?: boolean;
    noPriceChange?: boolean;
    noNoteAdd?: boolean;
    noModifierChange?: boolean;
  };
  reports?: {
    dailySales?: boolean;
    cashReport?: boolean;
...
type UserPermissions = {
  user: {
    ...
    allowedPriceLevelIds: string[];
    permissionsJson: PermissionsData | null;
  };
...
const PERMISSION_LABELS: Record<string, Record<string, string>> = {
  access: {
    ...
    delivery: "Dostęp do dostaw",
  },
  hall: {
    viewOtherOrders: "Podgląd innych zamówień",
    moveOrders: "Przenoszenie zamówień",
    joinTables: "Łączenie stolików",
    splitOrders: "Dzielenie rachunków",
  },
  operations: {
    discount: "Udzielanie rabatów",
    discountMax: "Max rabat (%)",
    freeItem: "Dawanie darmowych pozycji",
    cancelItem: "Anulowanie pozycji",
    cancelOrder: "Anulowanie zamówień",
    editSentItem: "Edycja wysłanych pozycji",
    reprintKitchen: "Ponowny wydruk do kuchni",
  },
  prohibitions: {
    noQuantityChange: "Zakaz zmiany ilości",
    noPriceChange: "Zakaz zmiany ceny",
    noNoteAdd: "Zakaz dodawania notatek",
    noModifierChange: "Zakaz zmiany modyfikatorów",
  },
  reports: {
    dailySales: "Raport dzienny",
    cashReport: "Raport kasowy",
    itemReport: "Raport produktów",
    staffReport: "Raport pracowników",
    deliveryReport: "Raport dostaw",
  },
  receipt: {
    openCashDrawer: "Otwieranie szuflady",
    payout: "Wypłaty z kasy",
    refund: "Zwroty",
    printReceipt: "Drukowanie paragonów",
    emailReceipt: "Email z paragonem",
  },
  order: {
    createOrder: "Tworzenie zamówień",
    addItems: "Dodawanie pozycji",
    removeItems: "Usuwanie pozycji",
    changeQuantity: "Zmiana ilości",
    applyCoupon: "Stosowanie kuponów",
  },
  delivery: {
    createDelivery: "Tworzenie dostaw",
    assignDriver: "Przypisywanie kierowców",
    changeStatus: "Zmiana statusu",
    editAddress: "Edycja adresu",
  },
  config: {
    editProducts: "Edycja produktów",
    editCategories: "Edycja kategorii",
    editTables: "Edycja stolików",
    editUsers: "Edycja użytkowników",
    editPrinters: "Edycja drukarek",
  },
};
  ...
  const updatePerm = (section: string, key: string, value: boolean | number) => {
    setLocalPerms((prev) => {
      ...
      return {
        ...current,
        [section]: {
          ...(current[section as keyof PermissionsData] ?? {}),
          [key]: value,
        },
      };
    });
      ...
      <header className="bg-gray-800 border-b border-gray-700 p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <select
              onChange={(e) => e.target.value && copyMutation.mutate(e.target.value)}
              className="bg-gray-700 border border-gray-600 rounded px-3 py-2"
              defaultValue=""
            >
              <option value="">Kopiuj z...</option>
              {(usersData?.users ?? [])
                .filter((u: { id: string }) => u.id !== id)
            ...
            <button
              onClick={() =>
                saveMutation.mutate({
                  permissionsJson: localPerms ?? undefined,
                  autoLogoutSec: autoLogoutSec ?? data.user.autoLogoutSec,
                })
              }
        ...
        <div className="bg-gray-800 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select
                value={autoLogoutSec ?? data.user.autoLogoutSec ?? ""}
                onChange={(e) => setAutoLogoutSec(e.target.value ? parseInt(e.target.value) : null)}
                  ...
                  <div key={key} className="flex items-center justify-between bg-gray-700 px-3 py-2 rounded">
                    ...
                    {isNumber ? (
                      <input
                        ...
                        min={0}
                        max={100}
                        value={value as number ?? effectiveValue as number ?? 100}
                        onChange={(e) => updatePerm(section, key, parseInt(e.target.value) || 0)}
                        className="w-20 bg-gray-600 border border-gray-500 rounded px-2 py-1 text-right"
                      />
                    ) : (
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={value as boolean ?? effectiveValue as boolean ?? true}
                          onChange={(e) => updatePerm(section, key, e.target.checked)}
