 ...
 * No reservations, simplified timing — matches RoomView for PosPageClient.
 */
export function useFloorFromDexie(): { rooms: RoomView[]; isLoading: boolean } {
  const rooms = useLiveQuery(
    async () => {
      ...
      const [allRooms, allTables, openOrders, allOrderItems] = await Promise.all([
        db.rooms.where("isActive").equals(1).sortBy("sortOrder"),
      ...
      return allRooms.map((room) => {
        const roomTables = allTables
          .filter((t) => t.roomId === room.id && t.isAvailable)
